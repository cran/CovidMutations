...
globalProteinMut <- function(covid_annot = covid_annot, outdir = NULL, figure_Type = "heatmap", top = 10, country = "global"){
  ...
  if(country != "global"){
    ...
    if(figure_Type == "count"){
      ...
      if(is.null(outdir) == FALSE){
        ggsave(p,filename = paste("Top", top, country, "_protein_count.png", sep = ""),width = 12, height = 8, dpi=300, path = outdir)
      }
      if(is.null(outdir) == TRUE){
        print(p)
      }
    ...
    if(figure_Type == "count"){
      ...
      p<-ggplot(data=covid_annot,aes(x=qpos,color=variant))+
        ...
        labs(x="SARS-CoV-2 Genomic Postion",
             title = 'SARS-CoV-2: Mutation_Count')
      if(is.null(outdir) == FALSE){
        ggsave(p,filename = paste("Top", top, "_Global_protein_count.png", sep = ""),width = 12, height = 8, dpi=300, path = outdir)
      }
      if(is.null(outdir) == TRUE){
        print(p)
      }
